    ...
    <v-container>
      <v-col>
        ...
        >
          <template v-slot:top>
            <v-toolbar flat>
              <v-toolbar-title>อาคาร</v-toolbar-title>
              <v-spacer></v-spacer>
              <v-text-field
                class="float-right"
                v-model="search"
                    ...
                    <v-container>
                      <v-row>
                        <v-col cols="12" sm="6" md="4">
                          <v-text-field
                            v-model="editedItem.building_code"
                            label="รหัสอาคาร"
                          ></v-text-field>
                        </v-col>
                        <v-col cols="12" sm="6" md="4">
          ...
          <template v-slot:item.actives="{ item }">
            <v-switch v-model="item.building_status"></v-switch>
          </template>
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: "ชื่ออาคาร", value: "building_name" },
      { text: "ถูกสร้างขึ้นวันที่", value: "building_createon" },
    ...
    defaultItem: {
      ...
      building_createon: null
    }
  }),

  computed: {
    formTitle() {
      return this.editedIndex === -1 ? "สร้างอาคารใหม่" : "แก้ไขอาคาร";
    }
  },
  ...
  methods: {
    ...
    save() {
      if (this.editedIndex > -1) {
        ...
        this.editedItem.building_createon = new Date().getUTCSeconds();
        this.editedItem.building_status = 1;
